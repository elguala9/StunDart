...
```bash
dart run singleton_manager_generator --input lib --registry-output lib/src/main_injection.dart
```
...
- **Dependency Injection**: `@dependencyInjectable`/`singleton_manager` integration — `src/main_injection.dart` is generated by `singleton_manager_generator` and wires every DI-annotated class into `RegistryManager.instance`; `DualStunInjector` (in `src/factories/dual_stun_registry_wiring.dart`) additionally wires the IPv4/IPv6 sockets the graph needs
- **Socket Migration**: helpers to rebind a registered handler onto a freshly bound socket while preserving its STUN server config (`src/migration/`)
- **Comprehensive Testing**: 239 passing tests with full coverage
