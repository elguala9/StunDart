...
void main() {
  ...
  group('NATDetector - Basic Detection', () {
    test(
      'should detect NAT type successfully',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
    ...
    test(
      'should provide valid public IP and port',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
    ...
    test(
      'should include filtering and mapping behavior',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
        ...
        try {
          ...
          expect(NATMappingBehavior.values, contains(result.mappingBehavior));
  ...
  group('NATDetector - Multiple Servers', () {
    test(
      'should work with different STUN servers',
      () async {
        final servers = [
    ...
    test(
      'should maintain consistent results on repeated calls',
      () async {
        final results = <NATType>[];
  ...
  group('NATDetector - Diagnostics', () {
    test(
      'should provide detailed test diagnostics',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
  ...
  group('NATDetector - Edge Cases', () {
    test(
      'should handle invalid server gracefully',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
    ...
    test(
      'should handle unreachable server (TEST-NET-1)',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
  ...
  group('NATDetector - Socket Management', () {
    test(
      'should use the provided socket',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
    ...
    test(
      'should work with socket on specific interface',
      () async {
  ...
  group('NATDetector - Result Validation', () {
    ...
    test(
      'should have consistent NAT type and behaviors',
      () async {
        final socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 0);
