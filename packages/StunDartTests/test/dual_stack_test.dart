    ...
    for (final server in ipv6Servers) {
      try {
        ...
        try {
          ...
          expect(response.publicIp.isNotEmpty, isTrue, reason: 'Should receive an IPv6 address');
          expect(response.ipVersion, equals(IpVersion.v6));
          expect(response.publicIp.contains(':'), isTrue, reason: 'IPv6 addresses contain colons');
  ...
  test('Verify both IPv4 and IPv6 across multiple STUN servers', () async {
    ...
    for (final server in stunServers) {
      try {
        ...
        final input4 = (address: server, port: StunServers.defaultPort, socket: socket4);
    ...
    for (final server in stunServers) {
      try {
        ...
        final input6 = (address: server, port: StunServers.defaultPort, socket: socket6);
    ...
    expect(ipv4Addresses.isNotEmpty, isTrue, reason: 'Should get at least one IPv4 address');
    expect(ipv6Addresses.isNotEmpty, isTrue, reason: 'Should get at least one IPv6 address');
    
    // Verify all IPv4 addresses from different servers match
    if (ipv4Addresses.length > 1) {
      expect(ipv4Addresses.toSet().length, equals(1), 
